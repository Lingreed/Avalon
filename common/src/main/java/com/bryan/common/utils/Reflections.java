 ...
 * Function: 反射工具类 提供调用getter/setter方法, 访问私有变量, 调用私有方法, 获取泛型类型Class, 被AOP过的真实类等工具函数
 *
 * @author Bryan.Lin
 * @version 1.0
 * @date 2018/3/1
...
public class Reflections {
    ...
    public static Class<? extends Job> reflex(String path) {
        Class<? extends Job> aClaz = null;
        try {
            aClaz = (Class<? extends Job>) Class.forName(path);
        } catch (ClassNotFoundException e) {
            logger.error("反射文件未找到", e);
        }
        return aClaz;
    }
